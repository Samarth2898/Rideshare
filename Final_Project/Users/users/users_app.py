...
def add_user():
    ...
    body1 = {"table":"users","column":"*","where":"username = '{}'".format(req["username"])}
    body2 = {"table":"users","column":"'username','password'","insert":"'{}','{}'".format(req["username"],req["password"])}
    x = requests.post('http://54.90.15.252/api/v1/db/read', json=body1)
    if len(json.loads(x.text))==0:
...
def rem_user(user_name):
    ...
    x = requests.post('http://54.90.15.252/api/v1/db/write', json=b1)
    x = requests.post('http://54.90.15.252/api/v1/db/write', json=b2)
    return {},200
#3
...
def clear():
    ...
    with sqlite3.connect("users.db") as conn:
        ...
        c.execute(query)
        conn.commit()
        x = requests.post('http://cc-ass-3-678167101.us-east-1.elb.amazonaws.com/api/v1/db/clear')
        return {},200
    return {},400
"""
