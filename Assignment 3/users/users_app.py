...
def rem_user(user_name):
    ...
    if(x.text=="True"):
        ...
        x = requests.post('http://54.84.178.218/api/v1/db/write', json=b1)
        x = requests.post('http://54.84.178.218/api/v1/db/write', json=b2)
        return {},200
    else:return {},400
#3
...
def clear():
    ...
    with sqlite3.connect("users.db") as conn:
        ...
        c.execute(query)
        conn.commit()
        x = requests.post('http://54.84.178.218/api/v1/db/clear')
        return {},200
    return {},400
